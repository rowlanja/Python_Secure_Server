...
def ConnectionSetup():
    while True:
        if check is True:
            ...
            try:
                while 1 :
                    clientPH = client.recv(1024)
                    if(len(clientPH)>0):
                        recv_msg += clientPH
                        print("recvieved : ", recv_msg)
                    else :
                        break
            ...
            if tmpHash == clientPublicHash:
                ...
                fSend = eightByte + (":" + session + ":" + my_hash_public).encode()
                #fSend = clientPublic.encrypt(fSend, None)
                client.send(fSend + (":".encode()) + public)
                if clientPH != "":
...
if __name__ == "__main__":
    ...
    check = False
    color_print("[1] server connecting to IP & PORT 127.0.0.1:8080\n[2] Starting up server\n", color="blue", bold=True)
    host = "127.0.0.1"
    port = 8081
